...
adb -s "!DEVICE_SERIAL!" shell mkdir -p "/sdcard/Android/data/com.kymjs.ai.custard/js_temp"

for %%F in ("%FILE_PATH%") do set "TARGET_FILE=/sdcard/Android/data/com.kymjs.ai.custard/js_temp/%%~nxF"
...
if not "!ENV_FILE_PATH!"=="" (
    if exist "!ENV_FILE_PATH!" (
        for %%E in ("!ENV_FILE_PATH!") do set "TARGET_ENV_FILE=/sdcard/Android/data/com.kymjs.ai.custard/js_temp/%%~nxE"
...
if "!HAS_ENV_FILE!"=="true" (
    adb -s "!DEVICE_SERIAL!" shell "am broadcast -a com.kymjs.ai.custard.EXECUTE_JS -n com.kymjs.ai.custard/.core.tools.javascript.ScriptExecutionReceiver --include-stopped-packages --es file_path '!TARGET_FILE!' --es function_name '!FUNCTION_NAME!' --es params '!PARAMS!' --es env_file_path '!TARGET_ENV_FILE!' --ez temp_file true --ez temp_env_file true"
) else (
    adb -s "!DEVICE_SERIAL!" shell "am broadcast -a com.kymjs.ai.custard.EXECUTE_JS -n com.kymjs.ai.custard/.core.tools.javascript.ScriptExecutionReceiver --include-stopped-packages --es file_path '!TARGET_FILE!' --es function_name '!FUNCTION_NAME!' --es params '!PARAMS!' --ez temp_file true"
)
