...
function Start-AgentAndProbeReady {
    ...
    $previousBindOverride = $env:CUSTARD_BIND_ADDRESS_OVERRIDE
    try {
        if ([string]::IsNullOrWhiteSpace($BindAddressOverride)) {
            Remove-Item Env:CUSTARD_BIND_ADDRESS_OVERRIDE -ErrorAction SilentlyContinue
        }
        else {
            $env:CUSTARD_BIND_ADDRESS_OVERRIDE = $BindAddressOverride
        }
        ...
        if ($null -eq $previousBindOverride) {
            ...
            $env:CUSTARD_BIND_ADDRESS_OVERRIDE = $previousBindOverride
        }
    }
...
try {
    ...
    Write-Log "INFO" "===== custard_pc_agent.bat ====="
