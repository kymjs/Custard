...
data class MCPPackage(
    ...
    companion object {
        ...
        fun fromServer(context: Context, serverConfig: MCPServerConfig): MCPPackage? {
            ...
            com.kymjs.ai.custard.util.AppLogger.d(TAG, "正在连接到MCP服务器: ${serverConfig.name}")

            try {
                // 尝试连接
                val connected = runBlocking { bridgeClient.connect() }
                if (!connected) {
                    com.kymjs.ai.custard.util.AppLogger.w(TAG, "无法连接到MCP服务器: ${serverConfig.name}")
                    return null
                }

                com.kymjs.ai.custard.util.AppLogger.d(TAG, "成功连接到MCP服务器: ${serverConfig.name}，开始获取工具列表")

                // 获取工具列表
                val jsonTools = runBlocking { bridgeClient.getTools() }
                if (jsonTools.isEmpty()) {
                    com.kymjs.ai.custard.util.AppLogger.w(TAG, "MCP服务器 ${serverConfig.name} 没有提供任何工具")
                    // 不要因为没有工具就返回null
                    // 返回一个包含空工具列表的有效包
                    com.kymjs.ai.custard.util.AppLogger.d(TAG, "创建不包含工具的MCP包 - 服务已连接但没有工具")
                    return MCPPackage(serverConfig, emptyList())
                }

                com.kymjs.ai.custard.util.AppLogger.d(TAG, "成功从MCP服务器获取 ${jsonTools.size} 个工具")
                ...
                val mcpTools =
                        jsonTools.mapNotNull { jsonTool ->
                            try {
                                ...
                                com.kymjs.ai.custard.util.AppLogger.e(TAG, "解析MCP工具时出错: ${e.message}")
                                null
                            }
                        }
                ...
                com.kymjs.ai.custard.util.AppLogger.d(TAG, "成功创建MCP包，包含 ${mcpTools.size} 个工具，保持连接活跃")
                return MCPPackage(serverConfig, mcpTools)
            } catch (e: Exception) {
                com.kymjs.ai.custard.util.AppLogger.e(TAG, "创建MCP包时出错: ${e.message}", e)
                // 只有在发生异常时才断开连接
