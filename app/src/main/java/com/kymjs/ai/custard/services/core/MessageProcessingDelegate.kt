...
import com.kymjs.ai.custard.core.tools.agent.PhoneAgentJobRegistry
import com.kymjs.ai.custard.data.model.*
import com.kymjs.ai.custard.data.model.InputProcessingState as EnhancedInputProcessingState
import com.kymjs.ai.custard.data.model.PromptFunctionType
import com.kymjs.ai.custard.util.stream.SharedStream
import com.kymjs.ai.custard.util.stream.share
...
import com.kymjs.ai.custard.ui.floating.ui.fullscreen.XmlTextProcessor
        ...
        coroutineScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val (provider, modelName) = try {
                    service.getProviderAndModelForFunction(com.kymjs.ai.custard.data.model.FunctionType.CHAT)
                } catch (e: Exception) {
        ...
        try {
            ...
            withContext(Dispatchers.IO) {
                ...
                if (isWaifuModeEnabled && WaifuMessageProcessor.shouldSplitMessage(finalContent)) {
                    ...
                    val (provider, modelName) = try {
                        getEnhancedAiService()?.getProviderAndModelForFunction(com.kymjs.ai.custard.data.model.FunctionType.CHAT)
                            ?: Pair("", "")
                    } catch (e: Exception) {
                        AppLogger.e(TAG, "获取provider和model信息失败: ${e.message}", e)
                        Pair("", "")
