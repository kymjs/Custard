...
import com.kymjs.ai.custard.core.tools.LocationData
...
import java.io.File
import com.kymjs.ai.custard.services.notification.CustardNotificationStore
...
open class StandardSystemOperationTools(private val context: Context) {
    ...
    suspend fun listInstalledApps(tool: AITool): ToolResult {
        val includeSystemApps = tool.parameters.find { it.name == "include_system_apps" }?.value?.toBoolean() ?: false
        return try {
    ...
    open suspend fun getNotifications(tool: AITool): ToolResult {
        ...
        val includeOngoing = tool.parameters.find { it.name == "include_ongoing" }?.value?.toBoolean() ?: false
