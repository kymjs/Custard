package com.kymjs.ai.custard.data.repository

import android.content.Context
import androidx.compose.ui.graphics.Color
import com.kymjs.ai.custard.R
import com.kymjs.ai.custard.data.db.ObjectBoxManager
import com.kymjs.ai.custard.data.model.Memory
import com.kymjs.ai.custard.data.model.MemoryLink
import com.kymjs.ai.custard.data.model.MemoryTag
import com.kymjs.ai.custard.data.model.MemoryTag_
import com.kymjs.ai.custard.data.model.Memory_
import com.kymjs.ai.custard.data.model.DocumentChunk
import com.kymjs.ai.custard.data.model.Embedding
import com.kymjs.ai.custard.data.model.CloudEmbeddingConfig
import com.kymjs.ai.custard.data.model.DimensionCount
import com.kymjs.ai.custard.data.model.EmbeddingDimensionUsage
import com.kymjs.ai.custard.data.model.EmbeddingRebuildProgress
import com.kymjs.ai.custard.data.preferences.MemorySearchSettingsPreferences
import com.kymjs.ai.custard.services.CloudEmbeddingService
...
import com.kymjs.ai.custard.data.model.MemoryExportData
import com.kymjs.ai.custard.data.model.SerializableMemory
import com.kymjs.ai.custard.data.model.SerializableLink
import com.kymjs.ai.custard.data.model.ImportStrategy
import com.kymjs.ai.custard.data.model.MemoryImportResult
import com.kymjs.ai.custard.data.model.MemorySearchDebugCandidate
import com.kymjs.ai.custard.data.model.MemorySearchDebugInfo
import com.kymjs.ai.custard.data.model.MemoryScoreMode
import com.kymjs.ai.custard.util.CustardPaths
import com.kymjs.ai.custard.util.TextSegmenter
...
class MemoryRepository(private val context: Context, profileId: String) {
    ...
    suspend fun deleteMemory(memoryId: Long): Boolean = withContext(Dispatchers.IO) {
        ...
        if (memory.isDocumentNode) {
            if (memory.chunkIndexFilePath != null) {
                try {
                    ...
                    if (indexFile.exists()) {
                        if (indexFile.delete()) {
                            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Deleted chunk index file: ${indexFile.path}")
                        } else {
                            com.kymjs.ai.custard.util.AppLogger.w("MemoryRepo", "Failed to delete chunk index file: ${indexFile.path}")
                        }
                    }
                } catch (e: Exception) {
                    com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Error deleting chunk index file for memory ID $memoryId", e)
                }
            }
            // 删除关联的区块
            ...
            if (chunkIds.isNotEmpty()) {
                ...
                com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Deleted ${chunkIds.size} associated chunks for document.")
            }
        }
        ...
        if (linkIdsToDelete.isNotEmpty()) {
            ...
            com.kymjs.ai.custard.util.AppLogger.d(
                "MemoryRepo",
    ...
    private fun cleanupDanglingLinksIfNeeded(force: Boolean = false) {
        ...
        if (danglingLinkIds.isNotEmpty()) {
            ...
            com.kymjs.ai.custard.util.AppLogger.w(
                "MemoryRepo",
    ...
    suspend fun linkMemories(
        ...
        if (existingLink != null) {
            // 链接已存在，可以选择更新或直接返回
            // 这里我们选择直接返回，不创建重复链接
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Link already exists from memory ${source.id} to ${target.id} with type $type")
            return@withContext
        }
    ...
    private suspend fun runSearchMemoriesWithDebug(
        ...
        if (memoriesToSearch.isEmpty()) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "No memories found in folder '$folderPath' to search.")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = buildDebug(
                    keywords = keywords,
        ...
        if (reverseContainmentResults.isNotEmpty()) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Reverse containment: ${reverseContainmentResults.size} matches")
        }
        ...
        if (effectiveSemanticWeight > 0.0f && cloudConfig.isReady()) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "--- Starting Semantic Search for ${keywords.size} keywords ---")
            keywords.forEach { keyword ->
                val queryEmbedding = generateEmbedding(keyword, cloudConfig)
                if (queryEmbedding == null) {
                    com.kymjs.ai.custard.util.AppLogger.w("MemoryRepo", "Failed to generate embedding for: '$keyword'")
                    return@forEach
                }
            ...
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "--- Semantic Search Completed ---")
        }
        ...
        if (edgesTraversed > 0) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Graph expansion: ${edgesTraversed} edges traversed")
        }
        ...
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Final results: ${filteredScores.size}/${scores.size} above threshold")
        ...
        sortedScoresForLogging.take(3).forEach { (id, score) ->
            ...
            if (memory != null) {
                com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "  Top: [${memory.title}] = ${String.format("%.4f", score)}")
            }
        }
        ...
        if (filteredScores.isEmpty()) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "No memories above relevance threshold")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = debugInfo
            )
    ...
    suspend fun searchChunksInDocument(memoryId: Long, query: String): List<DocumentChunk> = withContext(Dispatchers.IO) {
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "--- Starting search in document (Memory ID: $memoryId) for query: '$query' ---")
        val memory = findMemoryById(memoryId) ?: return@withContext emptyList<DocumentChunk>().also {
            com.kymjs.ai.custard.util.AppLogger.w("MemoryRepo", "Document with ID $memoryId not found.")
        }
        if (!memory.isDocumentNode) {
        ...
        if (query.isBlank()) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Query is blank, returning all chunks sorted by index.")
        ...
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Combined and deduplicated results count: ${combinedResults.size}. Results are now ordered by relevance.")
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "--- Search in document finished ---")
    ...
    suspend fun getAllFolderPaths(): List<String> = withContext(Dispatchers.IO) {
        ...
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Total memories: ${allMemories.size}")
        val folderPaths = allMemories
            .map { normalizeFolderPath(it.folderPath) ?: context.getString(R.string.memory_uncategorized) }
            .distinct()
            .sorted()
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Unique folders: $folderPaths")
        folderPaths
    }
    ...
    suspend fun renameFolder(oldPath: String, newPath: String): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Failed to rename folder", e)
            false
        }
    }
    ...
    suspend fun moveMemoriesToFolder(memoryIds: List<Long>, targetFolderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Failed to move memories", e)
            false
        }
    }
    ...
    suspend fun createFolder(folderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Failed to create folder", e)
            false
        }
            }
    ...
    suspend fun mergeMemories(
        ...
        if (sourceMemories.size < 2) {
            com.kymjs.ai.custard.util.AppLogger.w("MemoryRepo", "Merge requires at least two unique source memories to be found. Found: ${sourceMemories.size} from titles: ${sourceTitles.joinToString()}.")
        ...
        try {
            ...
            com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Error during memory merge transaction.", e)
            return@withContext null
        }
    ...
    suspend fun deleteMemoriesByUuids(uuids: Set<String>): Boolean = withContext(Dispatchers.IO) {
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Attempting to delete memories with UUIDs: $uuids")
        if (uuids.isEmpty()) {
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "UUID set is empty, nothing to delete.")
        ...
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Found ${memoriesToDelete.size} memories to delete.")
        ...
        try {
            store.runInTx {
                ...
                com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Found ${linkIdsToDelete.size} unique links and ${chunkIdsToDelete.size} chunks to delete.")

                // 2. 批量删除链接和区块
                if (linkIdsToDelete.isNotEmpty()) {
                    linkBox.removeByIds(linkIdsToDelete)
                    com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${linkIdsToDelete.size} links.")
                }
                if (chunkIdsToDelete.isNotEmpty()) {
                    chunkBox.removeByIds(chunkIdsToDelete)
                    com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${chunkIdsToDelete.size} chunks.")
                }

                // 3. 批量删除记忆本身
                memoryBox.removeByIds(memoryIdsToDelete.toList())
                com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${memoriesToDelete.size} memories.")
            }
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Transaction completed successfully.")
        } catch (e: Exception) {
            com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Error during bulk delete transaction.", e)
        ...
        for (memory in memoriesToDelete) {
            ...
            if (memory.isDocumentNode && memory.chunkIndexFilePath != null) {
                try {
                    ...
                    com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Error deleting chunk index file for memory UUID ${memory.uuid}", e)
                }
            }
        }
        com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Removed memories from vector index and cleaned up chunk index files.")
        ...
        memories.forEach { memory ->
            ...
            memory.links.forEach { link ->
                ...
                if (sourceId != null &&
                    ...
                    // com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Discarding edge: $sourceId -> $targetId
    ...
    suspend fun deleteFolder(folderPath: String) {
        withContext(Dispatchers.IO) {
            ...
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Deleted folder '$folderPath', moved ${memories.size} memories to uncategorized")
        }
    }
    ...
    suspend fun importMemoriesFromJson(
        ...
        try {
            ...
            com.kymjs.ai.custard.util.AppLogger.d("MemoryRepo", "Import completed: $newCount new, $updatedCount updated, $skippedCount skipped, $newLinksCount links")
            ...
            com.kymjs.ai.custard.util.AppLogger.e("MemoryRepo", "Failed to import memories", e)
            throw e
        }
    }
