  ...
  fun getSystemPrompt(
    ...
    val skillPackages = try {
        SkillRepository.getInstance(
            com.kymjs.ai.custard.core.application.CustardApplication.instance.applicationContext
  ...
  private fun getWorkspaceGuidelines(workspacePath: String?, workspaceEnv: String?, useEnglish: Boolean): String {
      ...
      return if (workspacePath != null) {
          if (useEnglish) {
              ...
              - Terminal mount note: common mounts include `/storage/emulated/0 -> /sdcard`, `/storage/emulated/0 -> /storage/emulated/0`, and app sandbox `/data/user/0/com.kymjs.ai.custard/files -> same path`.
              ...
              - 终端挂载说明：常见挂载包括 `/storage/emulated/0 -> /sdcard`、`/storage/emulated/0 -> /storage/emulated/0`，以及应用沙箱 `/data/user/0/com.kymjs.ai.custard/files -> 同路径`。
