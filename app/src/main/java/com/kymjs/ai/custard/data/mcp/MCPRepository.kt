package com.kymjs.ai.custard.data.mcp
...
class MCPRepository(private val context: Context) {
    ...
    private val pluginsBaseDir by lazy {
        ...
        val custardDir = File(downloadsDir, CUSTARD_DIR_NAME)
        val pluginsDir = File(custardDir, PLUGINS_DIR_NAME)
    ...
    suspend fun syncBridgeStatus() {
        withContext(Dispatchers.IO) {
            ...
            try {
                val bridge = com.kymjs.ai.custard.data.mcp.plugins.MCPBridge.getInstance(context)
