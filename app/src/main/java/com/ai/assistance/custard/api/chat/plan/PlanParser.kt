package com.ai.assistance.custard.api.chat.plan
...
object PlanParser {
    ...
    fun topologicalSort(graph: ExecutionGraph): List<TaskNode> {
        ...
        tasks.forEach { task ->
            task.dependencies.forEach { depId ->
                if (taskMap.containsKey(depId)) {
                    ...
                    AppLogger.w(TAG, CustardApplication.instance.getString(R.string.plan_task_dependency_not_exist, task.id, depId))
                }
            }
        }
        ...
        if (result.size != tasks.size) {
            AppLogger.e(TAG, CustardApplication.instance.getString(R.string.plan_circular_dependency) + "，无法进行拓扑排序")
            return emptyList()
        }
    ...
    fun validateExecutionGraph(graph: ExecutionGraph): Pair<Boolean, String> {
        ...
        if (taskIds.size != taskIds.toSet().size) {
            return false to CustardApplication.instance.getString(R.string.plan_task_id_not_unique)
        }
        ...
        graph.tasks.forEach { task ->
            task.dependencies.forEach { depId ->
                if (!validTaskIds.contains(depId)) {
                    return false to CustardApplication.instance.getString(R.string.plan_task_dependency_not_exist, task.id, depId)
                }
            }
        }
        ...
        if (sortedTasks.isEmpty() && graph.tasks.isNotEmpty()) {
            return false to CustardApplication.instance.getString(R.string.plan_circular_dependency)
        }
        
        return true to CustardApplication.instance.getString(R.string.plan_execution_graph_valid)
    }
} 