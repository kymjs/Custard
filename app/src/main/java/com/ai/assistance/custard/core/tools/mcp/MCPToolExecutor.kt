...
class MCPManager(private val context: Context) {
    ...
    private val clientCache =
            ConcurrentHashMap<String, com.ai.assistance.custard.data.mcp.plugins.MCPBridgeClient>()
    ...
    fun getOrCreateClient(
            serverName: String
    ): com.ai.assistance.custard.data.mcp.plugins.MCPBridgeClient? {
        // 检查缓存中是否已有客户端
        ...
        try {
            // 创建新的桥接客户端
            val client =
                    com.ai.assistance.custard.data.mcp.plugins.MCPBridgeClient(context, serverName)
