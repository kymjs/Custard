package com.ai.assistance.custard.data.converter
    ...
    override fun convert(content: String): List<ChatHistory> {
        return try {
            ...
            when (element) {
                ...
                else -> throw ConversionException(CustardApplication.instance.getString(R.string.generic_json_unsupported_format))
            }
        } catch (e: Exception) {
            throw ConversionException(CustardApplication.instance.getString(R.string.generic_json_parse_failed, e.message ?: ""), e)
        }
    }
            ...
            if (isConversationObject(firstElement)) {
                ...
                if (messages.isNotEmpty()) {
                    listOf(ChatHistory(
                        ...
                        group = CustardApplication.instance.getString(R.string.generic_json_import_from)
                    ))
                } else {
                    emptyList()
                }
