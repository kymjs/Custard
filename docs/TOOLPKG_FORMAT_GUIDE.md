...
**ToolPkg** 是 Custard 项目中用于打包和分发工具包的标准格式。它允许开发者将多个相关的工具脚本、资源文件和 UI 模块打包成一个单一的、易于分发和管理的文件。
...
```json
{
  ...
  "subpackages": [
    {
      ...
      "description": {
        "zh": "通过 Custard PC Agent 控制 Windows",
        "en": "Control Windows via Custard PC Agent"
      }
    }
  ],
  "ui_modules": [
  ...
  "resources": [
    {
      "key": "pc_agent_zip",
      "path": "resources/pc_agent/custard-pc-agent.zip",
...
/* METADATA
{
    ...
    "description": {
        "zh": "通过 HTTP 调用 Custard PC Agent 控制 Windows 电脑",
        "en": "Control a Windows PC through Custard PC Agent over HTTP"
    },
    ...
    "env": [
        {
            ...
            "description": {
                "zh": "Custard PC Agent 地址",
                "en": "Custard PC Agent URL"
            },
            "required": true
        }
    ],
