 ...
 */
export function adbShell(cmd: string): { stdout: string; stderr: string; success: boolean } {
  try {
    const full = `adb shell "${cmd.replace(/"/g, '\\"')}"`;
    const stdout = execSync(full, { encoding: "utf8", maxBuffer: 4 * 1024 * 1024 });
 ...
 */
export function dumpUiTree(displayId?: string): { xml: string; error?: string } {
  const dumpCmd =
 ...
 */
export function boundsToCenter(bounds: string): { x: number; y: number } | null {
 ...
 */
export function parseNodeAttrs(nodeTag: string): UINodeAttr {
 ...
 */
export function findAllNodeTags(xml: string): RegExpMatchArray[] {
  const nodeRe = /<node[^>]*>/g;
...
export function nodeMatches(
  ...
  const match = (
    ...
    if (partialMatch) return actual.includes(wanted);
    if (isResourceId)
      return (
        actual === wanted ||
  ...
  if (className != null && className !== "") {
    ...
    if (!cls) return false;
    if (partialMatch ? !cls.includes(want) : !cls.endsWith(want)) return false;
  }
  if (
    contentDesc != null &&
    contentDesc !== "" &&
 ...
 * Execute tap at (x, y).
 */
export function tap(x: number, y: number, displayId?: string): { success: boolean; error?: string } {
