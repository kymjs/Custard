...
import com.kymjs.ai.custard.terminal.utils.CacheManager
import com.kymjs.ai.custard.terminal.utils.UpdateChecker
import com.kymjs.ai.custard.terminal.utils.FtpServerManager
import com.kymjs.ai.custard.terminal.TerminalManager
import com.kymjs.ai.custard.terminal.data.MirrorSource
import com.kymjs.ai.custard.terminal.data.PackageManagerType
import com.kymjs.ai.custard.terminal.data.SourceConfig
import com.kymjs.ai.custard.terminal.data.SSHConfig
import com.kymjs.ai.custard.terminal.utils.SourceManager
import com.kymjs.ai.custard.terminal.utils.SSHConfigManager
import com.kymjs.ai.custard.terminal.utils.VirtualKeyboardConfigManager
import com.kymjs.ai.custard.terminal.utils.VirtualKeyboardLayoutConfig
...
class SettingsViewModel(
    ...
    private val _cacheSize = MutableStateFlow(application.getString(com.kymjs.ai.custard.terminal.R.string.cache_size_default))
    val cacheSize = _cacheSize.asStateFlow()

    private val _updateStatus = MutableStateFlow(application.getString(com.kymjs.ai.custard.terminal.R.string.update_status_default))
    ...
    // FTP服务器相关状态
    private val _ftpServerStatus = MutableStateFlow(application.getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_not_running))
    ...
    fun getCacheSize() {
        ...
        cacheSizeCalculationJob = viewModelScope.launch {
            ...
            _cacheSize.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.cache_calculating)
            try {
                // 调用新的 getCacheSize，并传入一个更新UI的回调
                val size = cacheManager.getCacheSize { currentSize ->
                    // 在回调中，实时更新UI状态
                    _cacheSize.value = "${getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.cache_calculating)} (${cacheManager.formatSize(currentSize)})"
                }
                _cacheSize.value = cacheManager.formatSize(size)
            } catch (e: Exception) {
                if (e is kotlinx.coroutines.CancellationException) {
                    _cacheSize.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.cache_calculation_cancelled)
                } else {
                    _cacheSize.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.cache_calculation_failed)
                }
            } finally {
    ...
    fun clearCache() {
        viewModelScope.launch {
            ...
            _cacheSize.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.environment_resetting)
            try {
                // 在清理缓存前停止FTP服务器
                ...
                _cacheSize.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.environment_reset_complete)
            } catch (e: Exception) {
                _cacheSize.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.environment_reset_failed, e.message ?: "")
            } finally {
    ...
    fun checkForUpdates() {
        viewModelScope.launch {
            _updateStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.checking_updates)
            when (val result = updateChecker.checkForUpdates(showToast = true)) {
                is UpdateChecker.UpdateResult.UpdateAvailable -> {
                    _updateStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.update_available, result.latestVersion, result.currentVersion)
                    _hasUpdateAvailable.value = true
                }
                is UpdateChecker.UpdateResult.UpToDate -> {
                    _updateStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.up_to_date, result.currentVersion)
                    _hasUpdateAvailable.value = false
                }
                is UpdateChecker.UpdateResult.Error -> {
                    _updateStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.update_check_failed, result.message)
                    _hasUpdateAvailable.value = false
                }
            }
        }
    ...
    fun startFtpServer() {
        viewModelScope.launch {
            ...
            _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_starting)
            try {
                ...
                if (success) {
                    ...
                    _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_start_failed_env)
                }
            } catch (e: Exception) {
                _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_start_failed, e.message ?: "")
            } finally {
    ...
    fun stopFtpServer() {
        viewModelScope.launch {
            _isManagingFtpServer.value = true
            _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_stopping_progress)
            try {
                val success = ftpServerManager.stopFtpServer()
                if (success) {
                    _isFtpServerRunning.value = false
                    _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_stopped)
                } else {
                    _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_stop_failed)
                }
            } catch (e: Exception) {
                _ftpServerStatus.value = getApplication<Application>().getString(com.kymjs.ai.custard.terminal.R.string.ftp_server_stop_failed_with_error, e.message ?: "")
            } finally {
                _isManagingFtpServer.value = false
            }
        }
