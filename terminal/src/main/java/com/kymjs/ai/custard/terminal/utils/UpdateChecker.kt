package com.kymjs.ai.custard.terminal.utils
...
class UpdateChecker(private val context: Context) {
    ...
    companion object {
        private const val GITHUB_API_URL = "https://api.github.com/repos/kymjs/CustardTerminal/tags"
        const val GITHUB_REPO_URL = "https://github.com/kymjs/CustardTerminal"
        const val GITHUB_RELEASES_URL = "https://github.com/kymjs/CustardTerminal/releases"
    }
                ...
                withContext(Dispatchers.Main) {
                    when (result) {
                        is UpdateResult.UpdateAvailable -> {
                            Toast.makeText(context, context.getString(com.kymjs.ai.custard.terminal.R.string.new_version_found, result.latestVersion), Toast.LENGTH_LONG).show()
                        }
                        is UpdateResult.UpToDate -> {
                        }
                        is UpdateResult.Error -> {
                            Toast.makeText(context, context.getString(com.kymjs.ai.custard.terminal.R.string.update_check_failed_short), Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
    ...
    fun openGitHubReleases() {
        try {
            ...
            Toast.makeText(context, context.getString(com.kymjs.ai.custard.terminal.R.string.cannot_open_browser), Toast.LENGTH_SHORT).show()
        }
    }
