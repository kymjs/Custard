/* METADATA
{
  ...
  "description": {
    "zh": "使用 Nano Banana API (基于Grsai的api服务/https://grsai.com/) 根据提示词画图，支持文生图和图生图（可传入参考图片URL或本地图片路径；本地图片会先上传到图床以获得公网URL），将图片保存到本地 /sdcard/Download/Custard/draws/ 目录，并返回 Markdown 图片提示。",
    "en": "Generate images using the Nano Banana API (via Grsai service / https://grsai.com/). Supports text-to-image and image-to-image (you can provide reference image URLs or local image paths; local images will be uploaded first to get public URLs). Saves images to /sdcard/Download/Custard/draws/ and returns a Markdown image reference."
  },
  "env": [
    "NANOBANANA_API_KEY",
    "BEEIMG_API_KEY"
...
const nanobananaDraw = (function () {
    ...
    async function draw_image(params: DrawImageParams): Promise<DrawImageResult> {
        ...
        hintLines.push("图片已生成并保存在本地 /sdcard/Download/Custard/draws/ 目录。");
    ...
    async function draw_image_wrapper(params: DrawImageParams) {
        try {
            ...
            complete({
                success: true,
                message: "图片生成成功，已保存到 /sdcard/Download/Custard/draws/，并返回 Markdown 图片提示。",
                data: result
            });
        } catch (error: unknown) {
